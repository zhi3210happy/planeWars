var _clientWidth=window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth; 
var _clientHeight=window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight; 
...
function moveHostilePlane() {
	...
	for(var i = 0; i < hPlanes.length; i++) {
		...
		if(parseInt(hPlanes[i].style.top) > _clientHeight) {
...
function moveDrop(dElement) {
	...
	dElement.times = setInterval(function() {
		if(!isPause) {
			...
			if(parseInt(dElement.style.left) <= 0 || parseInt(dElement.style.left) >=_clientWidth - dElement.offsetWidth) {
				xSpeed = xSpeed * -1;
			}
			if(parseInt(dElement.style.top) > _clientHeight) {
				clearInterval(dElement.times);
			}
		}
	}, 20)
...
function chooseHostilePlane() {
	...
	var hp;
	if(flightPath % 700 == 0) {
		pLeft = Math.floor(Math.random() * (_clientWidth - hPlanes[5].width));
		...
		pLeft = Math.floor(Math.random() * (_clientWidth - hPlanes[4].width));
		createHostilePlane(pLeft, hPlanes, 4)
	} else if(flightPath % 150 == 0) {
		pLeft = Math.floor(Math.random() * (_clientWidth - hPlanes[2].width));
		createHostilePlane(pLeft, hPlanes, 2)
	} else if(flightPath % 75 == 0) {
		pLeft = Math.floor(Math.random() * (_clientWidth - hPlanes[1].width));
		createHostilePlane(pLeft, hPlanes, 1)
	} else if(flightPath % 30 == 0) {
		pLeft = Math.floor(Math.random() * (_clientWidth - hPlanes[0].width));
...
function moveBoss() {
	...
	for(var i = 0; i < boss.length; i++) {
		...
		if(parseInt(boss[i].style.left) >= (_clientWidth-boss[i].offsetWidth) || parseInt(boss[i].style.left) <= 0) {
...
function chooseBoss(){
	...
	if(flightPath % 10000 == 0) {
		pLeft = Math.floor((_clientWidth - bosses[2].width)/2);
		...
		pLeft = Math.floor((_clientWidth - bosses[1].width)/2);
		...
		pLeft = Math.floor((_clientWidth - bosses[0].width)/2);
...
function createHostileBullet(bLeft, bTop, hostileBullet) { //坐标 和 子弹对象
	...
	hBullet.times = setInterval(function(){
		if (!isPause) {
			...
			if (parseInt(hBullet.style.top)>=_clientHeight || parseInt(hBullet.style.left)<=-parseInt(hBullet.style.width) || parseInt(hBullet.style.left)>=_clientWidth) {
				clearInterval(hBullet.times);
			}
		}
	},200);
...
function moveHostileBullet(xV, yV,hElement) { //x轴速度 y轴速度  定时器+自清除
	...
	hElement.times2 = setInterval(function(){
		if (!isPause) {
			...
			if (isDie ||parseInt(hElement.style.top)>=_clientHeight || parseInt(hElement.style.left)<=-parseInt(hElement.style.width) || parseInt(hElement.style.left)>=_clientWidth) {
				clearInterval(hElement.times2);
				try{
...
function movePlane(e) {
	var planeX = e.changedTouches[0].clientX - btg.offsetLeft - myPlane.offsetWidth / 2;
	var planeY = e.changedTouches[0].clientY - btg.offsetTop - myPlane.offsetHeight / 2;
